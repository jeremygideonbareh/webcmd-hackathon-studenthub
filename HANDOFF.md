...
| 2026-08-22 | Jeremy | **Phase 1-2 delivered on jeremy/delivery**: SQLite database.py + learning_engine.py (TDD, 18 tests), FastAPI dashboard (web/app.py + static UI), orchestrator.py pipeline. 29 tests passing. | Python 3.14 works with all deps (cp314 wheels available). PowerShell mangles inline python — use script files. |
| 2026-08-22 | Jeremy | **Supabase backend + Vercel deploy live**: created `atlas-studenthub` project (ap-south-1), 4 tables (reactions, preference_weights, digest_history, current_digest), delivery/supabase_store.py + orchestrator --push, Vercel serverless functions (api/digest.py, api/feedback.py, stdlib-only) + public/ static. **Live at webcmd-hackathon-studenthub.vercel.app** — E2E verified: dashboard 200, /api/digest reads Supabase, 👍 feedback persists weights (learning loop works across the internet). | Vercel CLI token validation broken on this machine + account is `limited` — deploy via GitHub push auto-deploy instead of API. PostgREST upserts need `Prefer: resolution=merge-duplicates`. Vercel env values can carry trailing newlines → `.strip()` env vars. Vercel project lives under "jeremygideonbareh's projects" team (token's default team can't see it). |
